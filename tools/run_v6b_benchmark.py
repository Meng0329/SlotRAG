"""V6b: Full-scale validation of question_grounded slotrag.

Tests the base slotrag method (now with question_grounded_retrieval=True)
on 200 questions per dataset — moderate scale before full eval.

Generation fixes from V5c remain active.
"""
...
def main():
    ...
    stage_name = "qo_v74_v6b_full_scale"
    ...
    sample_size = 200  # Moderate scale before full 20k
    seed = 314159
    ...
    for dataset in datasets:
        print(f"\n{'=' * 60}", flush=True)
        print(f"DATASET: {dataset} — loading {sample_size} questions & index", flush=True)
    ...
    reranker_http.close()
    (output_dir / "summary_v6b.json").write_text(json.dumps(stats, ensure_ascii=False, indent=2))
    print(f"\nSummary written to {output_dir / 'summary_v6b.json'}")
    print(f"Results: {stats['ok']}/{stats['total']} ok ({stats['ok'] / stats['total']:.1%})")
