#!/bin/bash
# Launch V6b: improved slotrag (question_grounded_retrieval) on 200 questions/dataset
DIR="/data/mzb/SlotRAG"
cd "$DIR" || exit 1
LOG="/tmp/hybrid-v6b.log"
> "$LOG"
...
" >> "$LOG" 2>&1
  ...
  .venv/bin/python -u tools/run_v6b_benchmark.py >> "$LOG" 2>&1 &
  PID=$!
