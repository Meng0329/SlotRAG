#!/bin/bash
# Launch V4 with retry loop for ext4 D-state
DIR="/data/mzb/SlotRAG"
cd "$DIR" || exit 1
LOG="/tmp/hybrid-v4.log"
...
from slotrag.benchmarking.metrics import score_record; from slotrag.benchmarking.methods import run_method
from slotrag.benchmarking.runner import _BudgetedRetriever, _BudgetedAgnes
import numpy as np, hashlib, json, time
print('IMPORTS WARM')
" >> "$LOG" 2>&1

  # Warm npy files - read fully into page cache
