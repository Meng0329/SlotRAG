...
Pairing and budget-exceeded accounting. Answer correctness is computed on the
matched valid pairs of a comparison: a question enters the paired statistic
only if both arms produced an ``ok'' record for it (retrieval, planning, and
generation succeeded within budget). Records that failed for a reason other
than reaching the retrieval-call ceiling---for example an `other` terminal
error---are excluded symmetrically whenever they fail on all arms, and are
retained (scored zero) if they fail on only one arm of a pair; Table~\ref{tab:overall}
reports the resulting matched-pair count $\#q$ alongside each cell. In the
reported sample there are four asymmetric cases, all favoring the chain arm:
on HotpotQA two static-arm records reached the retrieval-call ceiling while the
chain arm solved the same question within budget (both chain-correct), and one
static-arm record failed with an `other` terminal error while the chain arm
solved it (chain-incorrect); on 2Wiki one static-arm record failed with an
`other` error while the chain arm solved it (chain-incorrect). MuSiQue has no
asymmetric case. Under the alternative full-sample accounting in which these
budget-exceeded and single-arm-failed records are retained as zero-scored
answers (denominator $=$ all shared questions), the $\Delta$EM would be
HotpotQA $30.0\%\to45.0\%$ ($\Delta{=}{+}15.0$), 2Wiki
$45.0\%\to50.0\%$ ($\Delta{=}{+}5.0$), MuSiQue $33.3\%\to41.7\%$
($\Delta{=}{+}8.3$); the reported matched-pair $\Delta$EM in
Table~\ref{tab:overall} is $+6.3/+5.9/+9.1$. The two conventions agree in
direction and the matched-pair rule is the conservative one for the method on
the two budget-exceeded cells, so we report it throughout; the full-sample
view is disclosed in the artifact README.
