...
\begin{tabular}{lrr}
\toprule
& \textbf{Observed: No BE} & \textbf{Observed: BE} \\
\midrule
Predicted: Feasible ($\sum a_i \leq 8$) & TN = \textbf{76} & FN = \textbf{0} \\
Predicted: Infeasible ($\sum a_i > 8$) & FP = \textbf{128} & TP = \textbf{146} \\
\bottomrule
\end{tabular}
\caption{Feasibility confusion matrix on 350 frozen deep plans. The prediction is $\sum a_i > B$ (infeasible); the outcome is budget exhaustion (BE). Positive = infeasible, positive outcome = BE.}
\label{tab:confusion}
\end{table}
...
\bottomrule
\end{tabular}
\caption{Chain vs.\ flat on 350 deep plans. The chain importance law yields a marginal, statistically insignificant accuracy improvement (exact McNemar $p = 0.743$).}
\label{tab:ablation}
\end{table}

\noindent The chain variant uses importance $\tau = 2(i+1) - 1$ for slot $i$ in the longest chain, giving downstream slots higher allocation priority. On deep plans, the chain law yields $\Delta$EM $= +0.0086$ ($p = 0.743$, 95\% CI includes zero). The accuracy improvement is not statistically significant. The final system uses flat for simplicity: a dependency-weighted allocator that is not significantly more accurate than a uniform one does not justify its additional conceptual complexity.
...
\noindent Flat allocation reduces EM by 2.10 points on shallow plans ($p < 0.001$), with the harm concentrated in 2WikiMultiHop ($\Delta$EM $= -0.045$ on hops-0 questions) and near-zero on HotpotQA and MuSiQue shallow strata. This exploratory finding motivates the gate: applying the budget-aware planner to shallow plans does not help on average and can harm quality.
...
\toprule
\textbf{Structural regime} & \textbf{Evidence level} & \textbf{Flat $-$ Static $\Delta$EM} \\
\midrule
Shallow ($\texttt{hops} < 2$, $n = 8{,}085$) & \emph{exploratory} & $-0.021$ \quad (gray) \\
Deep ($\texttt{hops} \geq 2$, $n = 350$) & \emph{confirmatory} & $+0.077$ \quad (black) \\
\bottomrule
\end{tabular}
\caption{Regime-sensitive treatment effect of budget-aware flat allocation. The shallow effect is exploratory (large-scale retrospective trace); the deep effect is confirmatory (frozen matched-budget validation). Evidence levels are visually distinguished and not equated.}
\label{fig:gate}
\end{figure}
