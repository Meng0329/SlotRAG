...
\toprule
\textbf{Method} & \textbf{Decision input} & \textbf{Decision point} & \textbf{Learned?} & \textbf{Explicit plan?} & \textbf{Budget feasibility?} \\
\midrule
...
\textbf{SlotRAG} & \textbf{Typed plan} & \textbf{Allocation strategy} & \textbf{No} & \textbf{Yes} & \textbf{Yes (diagnosis)} \\
\bottomrule
\end{tabular}}%
\caption{Differential positioning. SlotRAG selects the physical allocation strategy for an already-compiled evidence plan and tests whether that allocation is globally feasible under the retrieval budget.}
