...
The chain importance law ($\tau = 2(i+1) - 1$) assigns higher weights to downstream slots in the dependency chain, reflecting the intuition that later evidence acquisition steps are more important because they depend on earlier bindings. On deep plans, this produces a marginal, statistically insignificant accuracy change ($\Delta$EM $= +0.0086$, $p = 0.743$). The dependency-weighted chain-specific allocator did not yield detectable accuracy gains over the uniform flat allocator. The final system uses flat allocation.
