...
All methods share one protocol: $\max\_\mathrm{steps}=8$, $\max\_\mathrm{retrieval\_calls}=8$, $\max\_\mathrm{llm\_calls}=96$. Budget-exceeded items score $0.0$; because evaluation uses exact-match scoring, budget-exhausted executions without a final answer receive $\text{EM} = 0$. This is the \emph{confirmatory} budget; an earlier permissive exploratory trace (used for mechanism discovery) used higher limits and is explicitly labeled throughout.
